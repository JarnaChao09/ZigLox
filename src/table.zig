...
pub const Table = struct {
    ...
    const AdpatedCtx = struct {
        captured_chars: []const u8,
        captured_hash: u32,

        pub fn init(chars: []const u8, chars_hash: u32) AdpatedCtx {
            return AdpatedCtx{
        ...
        pub fn hash(self: AdpatedCtx, _: @TypeOf(void)) u64 {
            return self.captured_hash;
        }

        pub fn eql(self: AdpatedCtx, _: @TypeOf(void), key: *ObjString) bool {
    ...
    pub fn findString(self: *Table, chars: []const u8, chars_hash: u32) ?*ObjString {
        return self.backing_map.getKeyAdapted(void, AdpatedCtx.init(chars, chars_hash));
    }
